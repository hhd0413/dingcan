...
class PayService():
    ...
    def createOrder(self,member_id,items=None,params=None):
        ...
        if continue_cnt >= len(items):
            ...
            resp['msg'] = '商品items为空'
            return resp
        yunprice = params['yun_price'] if params and 'yun_price' in params else 0
        ...
        yun_price = decimal.Decimal(yunprice)
        total_price = pay_price + yunprice
    ...
    def orderSuccess(self,pay_order_id = 0,params = None):
        '''
        将status（订单状态）置为1（支付完成）
        express_status（快递状态）置为-7（已付款待发货）
        更新售卖历史表中的内容
        '''
        try:
        ...
        if pay_order_items:
            for item in pay_order_items:
                ...
                if tmp_food_info:
                    ...
                    tmp_food_info.updatedtime = getCurrentDate()
