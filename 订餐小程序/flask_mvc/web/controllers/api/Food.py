...
def foodSearch():
    ...
    food_list = query.order_by(Food.total_count.desc(), Food.id.desc()).offset(offset).limit(pagesize).all()
    data_food_list = []
    if food_list:
        for item in food_list:
            tmp_data = {
    ...
    resp['data']['has_more'] = 0 if len(data_food_list) < pagesize else 1
    return jsonify(resp)
